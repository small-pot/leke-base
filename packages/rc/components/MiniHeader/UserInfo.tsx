...
function RoleSchoolList({userInfo}) {
    return userInfo.roleSchoolList && userInfo.roleSchoolList.map((ele,index) => {
        ...
        const active = (ele.roleId === userInfo.roleId && ele.schoolId === userInfo.schoolId) ? 'leke-role-current' : '';
        return <li className={active} key={index}>
            <a href={`${changeRole}&userId=${userInfo.userId}&roleId=${ele.roleId}&schoolId=${ele.schoolId}`}>{roleSchoolTitle}</a>
        </li>;
    });
}
        ...
        <div className='leke-miniHeader-user'>
            ...
            <ul className='leke-miniHeader-role-list'>
                <li className='leke-miniHeader-personal-center'>
                    <a target="_blank" rel="noreferrer" href={userCenter}>个人中心</a>
                </li>
                <RoleSchoolList userInfo={userInfo}/>
                <LearnCenter userInfo={userInfo} />
                <li className='leke-miniHeader-logout'><a href={logoutUrl}>退出登录</a></li>
            </ul>
        </div>
    );
}
