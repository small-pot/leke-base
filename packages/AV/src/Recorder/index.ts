 ...
 * @LastEditTime: 2021-02-05 14:10:23
 */
...
<div class='record-audio-wrap'>
    <div class='record-audio-container'></div>
    <div id='reWrap' class='record-reRecord'>重录</div>
</div>
`;
interface IRecorderConfig {
    ...
    player?: AudioPlayerOptions; //音频组件
    onStart?: () => void; //开始录音回调
...
class Recorder {
  ...
  initAudio() {
      ...
      if(this.isLoading){
          ...
          reRecorder.className  = 'record-loading';
      ...
      if(!this.isSuccess){
          let reRecorder = el.querySelector("#reWrap");
          reRecorder.className  = 'record-reUpload';
      ...
      let reRecorder = el.querySelector("#reWrap");
      reRecorder.className  = 'record-reRecord';
  ...
  audioUpload(e){
      const self = this;
      this.isLoading = true;
      this.cfg.loadSrc(e).then(function(src){
